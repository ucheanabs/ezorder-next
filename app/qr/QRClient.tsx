...
export default function QRClient({ href }: { href: string }) {
  ...
  useEffect(() => { if (canvasRef.current) QRCode.toCanvas(canvasRef.current, href, { width: 320, margin: 2 }); }, [href]);
  async function copy() { try { await navigator.clipboard.writeText(href); setCopied(true); setTimeout(()=>setCopied(false),1500);} catch {} }
  return (

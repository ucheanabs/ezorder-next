...
export default function OrderClient({ initialEventId }: { initialEventId: string }) {
  ...
  const [eventName, setEventName] = useState('Order');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (alive) { setMenu(j.menu || []); setEventName(j.event?.name || 'Order'); }
  ...
  const total = Object.entries(cart).reduce((sum, [id, qty]) => {
    const m = menu.find(x => x.id === id); return sum + (m ? m.price * qty : 0);
  }, 0);

  function inc(id: string) { setCart(c => ({ ...c, [id]: Math.min((c[id]||0)+1, 99) })); }
  function dec(id: string) { setCart(c => { const q=Math.max((c[id]||0)-1,0); const n={...c}; if(q===0) delete n[id]; else n[id]=q; return n; }); }
  ...
  async function placeOrder() {
    ...
    try {
      const res = await fetch('/api/orders', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ eventId: initialEventId, items }) });
      const j = await res.json(); if (!res.ok) throw new Error(j?.error || 'failed');
      setOrderId(j.id); setCart({});
    } catch { setError('Could not place order'); } finally { setPlacing(false); }
  }
